...
func fetchAndParseDSStore(base string, uris []string, client *downloader.Downloader, fs *[]string) {
	...
	for uri, body := range res {
		// common.Logger.Success("Fetched ", fmt.Sprintf("%s/%s", uri, dsStoreFile))
		common.Logger.Success("Fetched ", urlJoin(uri, dsStoreFile))
	...
	if len(nextUris) > 0 {
		...
		common.Logger.Debug("nextUris", nextUris)
...
func DSStore(s string, download bool) (string, error) {
	...
	if err := validator.New().Var(s, "url"); err == nil && strings.HasPrefix(s, "http") {
		...
		common.Logger.Normal("Fetch URL", base)
	...
	common.Logger.Success("Fetch File", s)
	_, err := os.Stat(s)
